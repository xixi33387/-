    ...
    const swimmerModes = ["锦鲤鱼群", "小鸭群", "一叶小舟"];
    const particles = [];
    const flowers = [];
    const halfFlowers = [];
    const buds = [];
    ...
    class KoiFish {
      ...
      update() {
        ...
        const maxLag = settings.swimmerMode === 2 ? 1 : 22;
        ...
        const groupOffset = settings.swimmerMode === 2 ? createVector(0, 0) : this.schoolOffset;
        const followPoint = p5.Vector.add(target, groupOffset);
        const distanceToFollow = p5.Vector.dist(this.position, followPoint);
        const settling = !pointerMoving && !this.isRoaming && (distanceToFollow > (settings.swimmerMode === 2 ? 7 : 18) || this.velocity.mag() > 0.42);

        if (!pointerMoving && settings.swimmerMode !== 2 && (this.isRoaming || !settling)) {
          if (!this.isRoaming) {
            this.idleWait = 0;
        ...
        if (!pointerMoving && settings.swimmerMode === 2 && !settling) {
        ...
        const activeSwim = settings.swimmerMode === 2
          ? createVector(0, 0)
        ...
        const followForce = settings.swimmerMode === 2 ? 6.2 : 3.15;
        const settleForce = settings.swimmerMode === 2 ? 0.64 : 0.42;
        ...
        const followSpeed = settings.swimmerMode === 2 ? 11.5 : 9.6;
        const settleSpeed = settings.swimmerMode === 2 ? 2.8 : 0.92;
        ...
        const settleDamping = settings.swimmerMode === 2
        ...
        if (this.shouldUpdateAngle(pointerMoving, targetDistance)) {
          const turnEase = settings.swimmerMode === 2 ? 0.12 : pointerMoving ? 0.24 : 0.16;
          this.angle = easeAngle(this.angle, this.velocity.heading(), turnEase);
        }

        if (settings.swimmerMode === 2) {
      ...
      drawSurface() {
        ...
        if (settings.swimmerMode === 2 && this.index !== 0) return;
    ...
    function applyKoiForces() {
      ...
      for (const fish of koi) {
        if (settings.swimmerMode === 2 && fish.index !== 0) continue;
        ...
        const radius = min(width, height) * (settings.swimmerMode === 2 ? 0.135 : settings.swimmerMode === 1 ? 0.132 * duckClearance : 0.105);
        const strengthScale = settings.swimmerMode === 2 ? 1.18 : settings.swimmerMode === 1 ? 1.22 * duckClearance : 1;
    ...
    function setupControls() {
      ...
      const updateSwimmerLabel = () => {
        ...
        swimmerValue.value = `${settings.swimmerMode + 1}/3`;
      };
      ...
      swimmerControl.addEventListener("click", () => {
        ...
        if (settings.swimmerMode === 2 && koiTrail.length > 0 && koi.length > 0) {
